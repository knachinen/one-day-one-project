...
import { sendEmergencySMS, callEmergencyNumber, generateEmergencyMessage } from '../utils/communication';
import { useContactStore } from '../store/useContactStore';
import { COLORS, SPACING, FONT_SIZE, LAYOUT } from '../constants/theme';

export const MainScreen = () => {
    const navigation = useNavigation<any>();
    const { coords, address, loading, error, accuracy, refreshLocation } = useLocation();
    ...
    const handleEmergencyActivate = useCallback(() => {
        const { coords: currentCoords, address: currentAddress } = locationRef.current;
                ...
                {loading ? (
                    ...
                    <>
                        ...
                        <Text style={[
                            ...
                            {accuracy === 'high' ? '정확도 높음' : '정확도 낮음 (GPS 신호 약함)'}
                        </Text>
                    </>
                )}
            </View>
                ...
                {contacts.length > 0 ? (
                    contacts.slice(0, 2).map((contact, index) => (
                        <React.Fragment key={contact.id}>
                            ...
                            >
                                <Ionicons name="call" size={24} color={COLORS.white} style={styles.actionIcon} />
                                <Text style={styles.actionButtonText}>{contact.name}</Text>
                            </Pressable>
                            {index === 0 && contacts.length > 1 && <View style={{ width: SPACING.m }} />}
                        </React.Fragment>
                    ))
                ) : (
                    <Pressable
                        ...
                        accessibilityRole="button"
                    >
                        <Ionicons name="add-circle-outline" size={24} color={COLORS.white} style={styles.actionIcon} />
