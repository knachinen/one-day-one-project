...
*   [ ] Refine the streak calculation logic in `useStreak` hook for accuracy.
*   [ ] Add a `Streak` entity to the `useHabitStore` to store the current and longest streaks.
*   [ ] Implement a visual effect (e.g., a "sparkle" or "shake" animation) using Framer Motion on a fire icon next to the streak when it's updated.
*   [ ] Update the `HabitCard` component to display the longest streak.

## 2. Habit Tree Visualization (Sub-step 2.2)

*   [ ] Create a `HabitTree` component to display the habit tree graphic.
*   [ ] Implement the growth algorithm based on daily completion percentage.
*   [ ] Add an animation for leaves growing on the tree when a habit is checked.
*   [ ] Implement a special "flowering" animation for 7-day completion streaks.
