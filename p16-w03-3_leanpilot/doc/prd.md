...
- **Next.js API Routes (Server Actions)**
    ...
    - **특징**: Server Actions 활용, Edge-Optimized 아키텍처

**ORM**:

- **Drizzle ORM 0.45.0** (2025년 12월 4일 최신, libSQL dialect)
    ...
    - **동반 도구**:
        - Drizzle Kit (마이그레이션 - *현재 수동 스키마 동기화로 대체*)
...
1. **SQLite 3.51.1+** (로컬 파일, Bun 내장 `bun:sqlite` 및 `@libsql/client` 활용)
    - 🔗 [공식 사이트](https://www.sqlite.org/)
    - **장점**: 설정 불필요, 단일 파일, 백업 용이, Docker 불필요
    - **단점**: 동시 쓰기 제한적 (MVP에 적합)
    - **적합**: MVP, 소규모 프로젝트
...
```env
# 데이터베이스
DATABASE_URL="file:./sqlite.db" # 로컬 SQLite 파일 경로
...
백엔드:
  ...
  - Next.js API Routes (Server Actions)
  - Drizzle ORM 0.45.0 (libSQL dialect)
  - Zod 3.24+
  - Lucia Auth 3+

데이터베이스:
  - SQLite (로컬 파일, @libsql/client)
