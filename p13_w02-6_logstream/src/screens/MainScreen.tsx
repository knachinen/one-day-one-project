...
const LogItem = React.memo(({ item, onPress }: { item: LogEntry, onPress: (item: LogEntry) => void }) => {
    const color =
        item.level === 'E' || item.level === 'F' ? '#ff5252' :
            item.level === 'W' ? '#ffab40' :
                item.level === 'D' ? '#448aff' :
                    '#69f0ae';
        ...
        <TouchableOpacity onPress={() => onPress(item)}>
            <View style={styles.logRow}>
                <Text style={styles.logText}>
                    <Text style={{ color }}>{item.level}/{item.tag}: </Text>
...
export const MainScreen = () => {
    ...
    const onLogPress = useCallback((item: LogEntry) => {
        ...
        <View style={styles.container}>
            <View style={styles.header}>
                ...
                <View style={styles.controls}>
                    <TouchableOpacity
                        style={[styles.button, isCapturing ? styles.btnStop : styles.btnStart]}
...
const styles = StyleSheet.create({
    ...
    header: { padding: 10, backgroundColor: '#2d2d2d', flexDirection: 'row', alignItems: 'center' },
    search: { flex: 1, backgroundColor: '#3d3d3d', color: '#fff', padding: 8, borderRadius: 5, marginRight: 10 },
    controls: { flexDirection: 'row' },
    button: { padding: 8, backgroundColor: '#555', borderRadius: 5, marginLeft: 5 },
