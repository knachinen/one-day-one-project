...
export const SettingsScreen = () => {
    ...
    const [filename, setFilename] = useState(`log-${new Date().toISOString().slice(0, 10)}.txt`);
    ...
    const loadFiles = async () => {
        try {
            const result = await FileSystem.readDirectoryAsync((FileSystem as any).documentDirectory!);
    ...
    const saveLogs = async () => {
        ...
        const content = logs.map(l => `${l.timestamp} ${l.level}/${l.tag}: ${l.message}`).join('\n');
        try {
...
const styles = StyleSheet.create({
    ...
    fileRow: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 15, borderBottomWidth: 1, borderColor: '#eee' },
